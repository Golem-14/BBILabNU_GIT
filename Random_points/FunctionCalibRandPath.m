function FunctionCalibRandPath(path, N_RI, N_Val, sensor_trace, savedata, code_name, set, calibr, modif, range_start, range_end, number_of_points, exclude_min, exclude_max)
...
%   minprom       - Minimum peak prominence for findpeaks (used if findpeaks were active).
%   maxpeakw      - Maximum peak width for findpeaks (used if findpeaks were active).
%   minpeakd      - Minimum peak distance for findpeaks (used if findpeaks were active).
...
%   range_start   - Start index for random wavelength selection.
%   range_end     - End index for random wavelength selection.
%   number_of_points - Number of random points to generate.
%   exclude_min   - Minimum index to exclude from random selection.
%   exclude_max   - Maximum index to exclude from random selection.
%
% Example call:
%   % Note: minprom, maxpeakw, minpeakd are included but currently ignored
%   % by the active random point selection logic.
%   FunctionCalibRand(6, 20, 6, 0, inf, 0, 0, "LV30", 18, 27, 0, 1000, 20000, 100, 5000, 7000)
%
...
%If FBG is present here you can cut it manually
% --- Load Data ---
kk = 1;
for ii = 1:N_RI
    for jj = 1:N_Val
...
% Getting preliminary peak and valley values and positions using random sampling
% The following commented block shows the original findpeaks logic:
...
% 2. Generate random numbers in the overall range.
initial_A = randi([range_start, range_end], 1, number_of_points);
% 3. Filter out the values within the excluded range
valid_indices = (initial_A < exclude_min) | (initial_A > exclude_max);
filtered_A = initial_A(valid_indices);
pre_peak_locs = unique(filtered_A);
dataframe1 = CHF_sens_clean(1,:);
pre_peaks = dataframe1(pre_peak_locs); % Pre-calculated values at random indices
% Extracting valley locations and values
initial_A = randi([range_start, range_end], 1, number_of_points);
% 3. Filter out the values within the excluded range
valid_indices = (initial_A < exclude_min) | (initial_A > exclude_max);
filtered_A = initial_A(valid_indices);
pre_valley_locs = unique(filtered_A);
dataframe2 = -CHF_sens_clean(1,:);
pre_valleys = dataframe2(pre_valley_locs); % Pre-calculated values at random indices
% Plotting peaks
figure
...
if ~isempty(pre_peak_locs)
    % FIX: The lines below caused the error by overwriting pre_peaks with the 
    % output of findpeaks (which has a different number of elements than pre_peak_locs)
...
if ~isempty(pre_valley_locs)
    % FIX: Removing the incorrect findpeaks call here too.
...
if savedata == 1
    writematrix(pre_peak_locs, strcat(probe_name, '_pre_peak_locs', '.csv'))
    writematrix(pre_valley_locs, strcat(probe_name, '_pre_valley_locs', '.csv'))
    writematrix(sens_peaks, strcat(probe_name, '_sens_peaks', '.csv'))
    writematrix(sens_valleys, strcat(probe_name, '_sens_valleys', '.csv'))
    writematrix(r2_peaks, strcat(probe_name, '_r2_peaks', '.csv'))
    writematrix(r2_valleys, strcat(probe_name, '_r2_valleys', '.csv'))
end
end
