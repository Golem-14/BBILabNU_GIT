...
%%%%%% 
%%%%%% Logging Results to Text File (Sorted by Sensitivity)
%%%%%%

% 1. Create the full file path using the existing pathToData
log_name = sprintf('chan_%d_sens_%s_log_SORTED.txt', sensor_trace, sensor_name);
log_full_path = fullfile(pathToData, log_name);

% 2. Sort the Data before writing
% Sort Peaks: Highest sensitivity first
[good_peaks_sens, p_sort_idx] = sort(good_peaks_sens, 'descend');
good_peaks_indices = good_peaks_indices(p_sort_idx);

% Sort Valleys: Most negative (highest magnitude) sensitivity first
[good_valleys_sens, v_sort_idx] = sort(good_valleys_sens, 'ascend');
good_valleys_indices = good_valleys_indices(v_sort_idx);
...
% 6. Write the data columns
for i = 1:max_rows
    % --- Handle Peak data (Now Sorted) ---
    if i <= num_peaks
...
fprintf('Log saved (Sorted by Sensitivity) to: %s\n', log_full_path);