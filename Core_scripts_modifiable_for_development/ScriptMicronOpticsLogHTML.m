...
% Save your main sensitivity figure
exportgraphics(gcf, fullfile(pathToData, sprintf('%s_plot.png', sensor_name)), 'Resolution', 300);


% 1. Setup names
html_filename = fullfile(pathToData, sprintf('Report_%s.html', sensor_name));
fileID = fopen(html_filename, 'w'); % 'w' overwrites to create a fresh report

% 2. Write HTML Header and Styles
fprintf(fileID, '<html><head><style>');
fprintf(fileID, 'body {font-family: monospace; background-color: #f4f4f4; padding: 20px;}');
fprintf(fileID, 'table {border-collapse: collapse; background: white; margin-bottom: 20px;}');
fprintf(fileID, 'th, td {border: 1px solid #ccc; padding: 8px; text-align: left;}');
fprintf(fileID, 'th {background-color: #eee;}');
fprintf(fileID, '.header-info {background: #fff; padding: 15px; border-left: 5px solid #333; margin-bottom: 20px;}');
fprintf(fileID, '</style></head><body>');

% 3. Write Sensor Info
fprintf(fileID, '<h1>Sensor Analysis Report</h1>');
fprintf(fileID, '<div class="header-info">');
fprintf(fileID, '<strong>Timestamp:</strong> %s<br>', char(datetime('now')));
fprintf(fileID, '<strong>Sensor Name:</strong> %s<br>', sensor_name);
fprintf(fileID, '<strong>Channel:</strong> %d<br>', sensor_trace);
fprintf(fileID, '</div>');

% 4. Insert the Image
img_name = sprintf('%s_plot.png', sensor_name);
fprintf(fileID, '<h3>Sensitivity Plot</h3>');
fprintf(fileID, '<img src="%s" width="800"><br><br>', img_name);

% 5. Write the Data Table
fprintf(fileID, '<h3>Data Summary</h3>');
fprintf(fileID, '<table><tr><th>Pk Idx</th><th>Pk Loc</th><th>Pk Sens</th><th>Pk R2</th><th>Vl Idx</th><th>Vl Loc</th><th>Vl Sens</th><th>Vl R2</th></tr>');
...
for i = 1:max_rows
    % Logic for data (reusing your existing logic)
    p_vals = {'---','---','---','---'};
    if i <= num_peaks
        idx_p = good_peaks_indices(i);
        p_vals = {num2str(idx_p), num2str(pre_peak_locs(1, idx_p)), sprintf('%.4f', good_peaks_sens(i)), sprintf('%.4f', r2_peaks(idx_p))};
    end
    
    v_vals = {'---','---','---','---'};
    if i <= num_valleys
        idx_v = good_valleys_indices(i);
        v_vals = {num2str(idx_v), num2str(pre_valley_locs(1, idx_v)), sprintf('%.4f', good_valleys_sens(i)), sprintf('%.4f', r2_valleys(idx_v))};
    end
    
    fprintf(fileID, '<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>', ...
        p_vals{:}, v_vals{:});
end
